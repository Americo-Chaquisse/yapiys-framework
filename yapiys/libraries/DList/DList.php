...
class DList {
    ...
    public static function mongoAggregate($columns,$header_cols=false,$collection=false,$find=false,$default_sort=false){
        ...
        //Ordering
        if(DList::isOrderingActive()){


            $orderby = DList::getOrderingColumn($header_cols);
            $order = DList::getOrderingOrder();
    ...
    public static function buildFindQuery($table_or_class,$find_by_cols,$header_cols=false,$ands=array()){
        ...
        $final_find_columns = array();

        //This guarantees that rows with NULL columns values with also be considered
        foreach($find_by_cols as $col){

            $col_with_if = "IFNULL($col,'')";
            $final_find_columns[] = $col_with_if;
        ...
        foreach($ands as $and_key => $and_value){

            if(is_array($and_value)){

                $signal=array_keys($and_value)[0];
                $value = $and_value[$signal];

                $query = $query.' AND '.$and_key.$signal.$value;


            }else{

                $query = $query.' AND '.$and_key."=".$and_value;
        ...
        if(isset($_GET['ordering_by'])&&isset($_GET['ordering'])){
            ...
            $order = false;
            $columnName = DList::getOrderingColumn($header_cols);
            $ordering = DList::getOrderingOrder();
