	...
	implements MongoRecord
{
	...
	//Actualiza um determinado registo
	public function update(){
		...
		if(is_object($id)){
			$status = $collection->update(array("_id" => $id), $attr);
		} else {
			$status = $collection->update(array("_id" => new MongoId($id['$id'])), $attr);
		}
		return $status;
	}
	...
	public static function findAll($query = array(), $options = array(), $asArray = false)
	{
	...
	protected static function getCollection()
	{
		...
		$mongoConnections = $className::$connection->getConnections();

		if(empty($mongoConnections)) $className::$connection->connect();
